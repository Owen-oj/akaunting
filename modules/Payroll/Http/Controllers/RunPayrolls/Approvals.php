...
class Approvals extends Controller
{
    ...
    public function edit(PayCalendar $payCalendar, RunPayroll $runPayroll)
    {
    ...
    public function update(PayCalendar $payCalendar, RunPayroll $runPayroll, Request $request)
    {
        $amount = 0;
        ...
        $payment = Transaction::updateOrCreate([
            ...
            'reference' => trans('payroll::run-payrolls.reference', ['id' => $runPayroll->id]),
        ]);
        ...
        $response = [
            'success' => true,
            'error' => false,
            'redirect' => route('payroll.run-payrolls.index'),
            'data' => [],
        ];
