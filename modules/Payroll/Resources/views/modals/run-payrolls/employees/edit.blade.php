{!! Form::open([
    ...
    <div class="card">
        <div class="card-body">
            <div class="row">
                {{ Form::dateGroup('from_date', trans('payroll::run-payrolls.from_date'), 'calendar', ['id' => 'from_date', 'required' => 'required', 'data-inputmask' => '\'alias\': \'yyyy-mm-dd\'', 'data-mask' => '', 'autocomplete' => 'off'], $run_payroll->from_date, 'col-md-4') }}

                {{ Form::dateGroup('to_date', trans('payroll::run-payrolls.to_date'), 'calendar', ['id' => 'to_date', 'required' => 'required', 'data-inputmask' => '\'alias\': \'yyyy-mm-dd\'', 'data-mask' => '', 'autocomplete' => 'off', 'min-date' => 'form.from_date'], $run_payroll->to_date, 'col-md-4') }}

                {{ Form::dateGroup('payment_date', trans('payroll::run-payrolls.payment_date'), 'calendar', ['id' => 'payment_date', 'required' => 'required', 'data-inputmask' => '\'alias\': \'yyyy-mm-dd\'', 'data-mask' => '', 'autocomplete' => 'off'], $run_payroll->payment_date, 'col-md-4') }}
            </div>
        </div>
    </div>
            ...
            <table class="table table-flush table-hover">
                <thead class="thead-light">
                    <tr class="row table-head-line">
                        ...
                        <th class="col-md-2">{{ trans_choice('payroll::general.positions', 1) }}</th>
                ...
                <tbody>
                    @foreach($employees as $item)
                        <tr class="row align-items-center border-top-1">
                            <td class="col-md-2 hidden-xs  border-0"><a href="{{ route('payroll.employees.show', $item->employee->id ) }}">{{ $item->employee->contact->name }}</a></td>
                            <td class="col-md-2 hidden-xs  border-0">{{ $item->employee->position->name }}</td>
                            <td class="col-md-2 hidden-xs  border-0">@money($item->employee->amount, setting('default.currency'), true)</td>
                            <td class="col-md-2 hidden-xs  border-0">@money($item->employee->total_benefits, setting('default.currency'), true),</td>
                            <td class="col-md-2 hidden-xs  border-0">@money($item->employee->total_deductions, setting('default.currency'), true)</td>
                            <td class="col-md-2 hidden-xs  border-0">@money($item->employee->totals, setting('default.currency'), true)</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
